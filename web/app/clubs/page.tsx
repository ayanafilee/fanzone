        ...
        <div className="space-y-6 pt-8">
            ...
            <div className="md:hidden space-y-4">
                {isLoading ? (
                    ...
                    <div key={club.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                        <div className="flex items-center gap-4 p-4">
                            <div className="relative w-20 h-20 flex-shrink-0 bg-gray-50 rounded-2xl flex items-center justify-center">
                                <Image src={club.logo_url} alt={club.name} fill className="object-contain p-2" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="text-lg font-black text-[#132A5B] tracking-tight truncate">{club.name}</h3>
                                <div className="flex items-center gap-2 mt-1">
                                    <MdEmojiEvents size={14} className="text-[#00A3E0] flex-shrink-0" />
                                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wider truncate">{getLeagueName(club.league_id)}</span>
                                </div>
                            </div>
                        </div>
